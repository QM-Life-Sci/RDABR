...
Tues
- Continuous predictors interactive lecture 
- HW read paper with continuous 
- In class - analyze fig legends

Thurs
- PS together in groups 

- take away assignment - code challenge independent & understanding questions
...
- Review problem set key: PS 2
- Three-week Check In: complete by end of day today (if you choose to)
    - We don't know who you are in the results
- PS 3 due Thursday
    - Key will be available on Canvas
- Progress Check 1 next week
    - Tuesday: Questions and review (feel free to submit ahead)
    - Thursday: In class working time
- Question for Thursday: Would you rather be able to speak all languages fluently or understand animals’ communication?
...
- *Is there* an effect (e.g., mean differences between groups)?
...
## (One of) the Goals of Statistics
...
## Think about your own lab experiences
...
- What is the probability of observing a difference this large *from sampling error only*?
...
## How large of a difference do you get from sampling error?

1. Assume long-winged and short-winged crickets are from *the same* distribution
2. Put all values from both morphs in one group
3. Randomly assign Long Wing and Short Wing labels to "observed" values
...
SS |> group_by(Wing_Morph) |> summarize(Mean = mean(Sim_CO2))
```
...
- Some models are very wrong and some less wrong.
- Your role as a scientist is to choose (and defend) a model, given your questions and the assumptions of the model.
...
- Real number line (rounding okay): bone lengths, age, mouse masses, oil content of corn
- Counts of species
...
- Treatment or observation groups: Ag/Non-Ag land use; long- or short-winged crickets
- Genotypes: *MM*, *Mm*, *mm*
:::
...
- (Almost) all data can be assigned to either continuous or categorical.
- The type of data will determine the statistical analysis.
...
- Bivariate normality: Separately, $x$ and $y$ are normally distributed
...
- Don't manually search for values of intercept ($\theta_0$) and slope ($\theta_1$) ^[Although we can.]
...
- `lm` is used to fit linear models, ... It can be used to carry out regression, ... analysis of variance and analysis of covariance ....
