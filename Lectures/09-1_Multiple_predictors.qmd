...
post_hoc <- glht(
  fm,
  mcp(Treatment = c("Eyes - Control = 0", "Knee - Control = 0"))
)
summary(post_hoc)
```
...
F_v_L +
  ...
  geom_mark_ellipse(
    aes(filter = Species == "Callimico goeldii", description = desc),
...
pred_Lactose <- data.frame(
  Lactose = seq(min(Milk$Lactose), max(Milk$Lactose), length.out = 200)
)
...
milk_res_lact +
  ...
  geom_mark_ellipse(
    aes(filter = Species == "Callimico goeldii"),
    label.fontsize = 8
  )
```
...
pred_Fat <- data.frame(
  Fat = seq(min(Milk$Fat), max(Milk$Fat), length.out = 200)
)
...
plot_grid(
  F_v_L + ggtitle("Estimate Fat coefficient"),
