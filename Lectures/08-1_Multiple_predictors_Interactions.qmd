---
title: "Unit 9: Multiple categories and multiple predictors"
html-math-method:
  method: mathjax
...
- PC 2 check-in
    - Due Thursday. Let us know if you need more time.
- Micropublication analysis plan due in a few weeks (4/21)
...
- Is **at least** one mean different?
    - Null hypothesis: the means *not different* than one another
...
![](https://i.imgur.com/X8kUEJq.jpg){fig-align="center" fig-alt="Photo of a person sleeping on an airplane."}
:::
...
1. Some proportion explained by group membership
1. Some proportion remains unexplained ("error" or "residual")
...
#| echo: false
#| fig-align: center
#| fig-alt: "Dot plot showing shift in circadian rhythm vs. light treatment group. The eye group appears to have a lower mean than the other two groups."

JL <- read_csv(
  "https://raw.githubusercontent.com/QM-Life-Sci/QMLS_1/refs/heads/main/data/JetLag.csv",
  show_col_types = FALSE
) |>
  mutate(
    Treatment = factor(Treatment, labels = c("Control", "Eyes", "Knee")),
    Treatment = fct_relevel(Treatment, "Knee", after = 1L)
  )

p1 <- ggplot(JL, aes(x = Treatment, y = Shift)) +
  geom_point(
    position = position_jitter(width = 0.1, seed = 43577),
    size = 4,
    color = "steelblue"
  ) +
...
#| echo: false
#| fig-align: center
#| fig-alt: "Dot plot showing shift in circadian rhythm vs. light treatment group. The eye group appears to have a lower mean than the other two groups. The means and standard errors are shown for each group."
...
- "At least of these means is different from the others."
...
`multcomp` package has all the tools for carrying out *post-hoc* tests [@Bretz2010-cy]
...
post_hoc <- glht(
  fm,
  mcp(
    Treatment = c("Eyes - Control = 0", "Knee - Control = 0")
  )
)
summary(post_hoc)
```
...
Pred <- tibble(Date = seq(min(DD$Date), max(DD$Date), length.out = 100))
Pred <- Pred |>
  mutate(Pred = predict(fm, newdata = Pred))

date_labs <- seq(ymd('2012-05-25'), ymd('2012-07-20'), by = '1 week') |>
  str_remove("2012-")
...
ggplot() +
  ...
  scale_x_continuous(limits = c(1, 9), breaks = 1:9, labels = date_labs) +
  labs(x = "Date of Birth", y = "Yearling Survival") +
  theme(axis.title = element_text(face = "bold"))
```
...
:::{.right}
Adapted from Tabachnick and Fidell [-@Tabachnick2019-tl]
:::
...
[(photo by Thomas Marent)]{.absolute bottom="5%" right="0%"}
...
NMR <- ggplot(M, aes(x = Mass, y = Energy, color = Caste)) +
  ...
  theme(legend.position = "inside", legend.position.inside = c(0.05, 1)) +
  labs(x = "ln Body Mass (g)", y = "ln Daily Energy Expenditure (kJ)")
NMR
```
