...
ggplot(BM, aes(x = Nitrogen, y = Biomass, color = Phosphorous)) +
  geom_point(
    position = position_jitter(width = 0.1, seed = 34234),
    size = 3
  ) +
...
int_plot1 <- ggplot(
  BM,
  aes(x = Nitrogen, y = Biomass, color = Phosphorous, group = Phosphorous)
) +
  geom_point(
    position = position_jitter(width = 0.1, seed = 34234),
    size = 3,
...
int_plot2 <- ggplot(
  BM,
  aes(x = Phosphorous, y = Biomass, color = Nitrogen, group = Nitrogen)
) +
  geom_point(
    position = position_jitter(width = 0.1, seed = 34234),
    size = 3,
...
BM |>
  summarize(mean = mean(Biomass), .by = c(Nitrogen, Phosphorous))
```
...
fm <- lm(Biomass ~ Nitrogen + Phosphorous + Nitrogen:Phosphorous, data = BM)
```
