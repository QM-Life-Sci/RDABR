...
source(
  "https://raw.githubusercontent.com/QM-Life-Sci/QMLS_1/refs/heads/main/Lectures/QMLS_functions.R"
)
...
## What have you learned about data visualization?

![](images/Sleep.png){fig-alt="Map showing the percentage of high school students who do not sleep enough."}
...
```{r}
#| fig-alt: "Scatterplot of Horn Length versus group (alive or dead). A photo of a horned lizard is inlayed on the top right."
...
liz <- image_read(
  "https://tpwd.texas.gov/huntwild/wild/images/reptiles/horned_lizardlarge.jpg"
)
...
P_HL <- HL |>
  ...
  geom_point(
    position = position_jitter(width = 0.2, seed = 423479),
    alpha = 0.5,
    size = 3
  ) +
...
```{r}
#| fig-alt: "Scatterplot of Horn Length versus group (alive or dead). The mean for each group is overlayed with a horizontal blue line."
...
```{r}
#| fig-alt: "Scatterplot of Horn Length versus group (alive or dead). The mean for each group is overlayed with a horizontal blue line. The 95% CI for each group is overlayed in red."
...
```{r}
#| fig-alt: "Scatterplot of Horn Length versus group (alive or dead). The mean for each group is overlayed with a horizontal blue line that spans the entire plot, showing the difference in means between groups. The effect size estimate is written between the blue lines and is 0.22."
...
P_HL +
  ...
  annotate(
    ...
    x = 1.5,
    y = mu.d + 0.5 * dd,
    label = paste0("Difference:", round(dd, 2), " mm"),
    size = 6
  )
...
::: {.column width="50%"}
![](https://i.imgur.com/EIfjPa0.jpg){fig-align="center" width=100% fig-alt="Image of two horned lizards being held in human hands."}
:::
...
```{r}
#| fig-alt: "Dotplot showing 180 random draws from the normal distribution. Points are stacked vertically when there is more than one value at the same point"

set.seed(34872394)
dots <- tibble(
  Horn_Length = rnorm(180, mean = mu.horn, sd = sd.horn),
...
```{r}
#| fig-alt: "Dotplot showing 180 random draws from the normal distribution. Points are stacked vertically when there is more than one value at the same point. 150 points are randomly assigned to the alive group and 30 to the dead group with different colors showing group membership."
...
```{r}
#| fig-alt: "Histogram of the difference in means for 100,000 simulations of two groups that are drawn from the same normal distribution."
...
```{r}
#| fig-alt: "Three panels of histograms. The right histogram shows the difference in means for 100,000 simulations of two groups that are drawn from the same normal distribution. Two stacked histograms on the left show the distribution of horn lengths for the alive group (top) and dead group(bottom) with vertical red lines showing the mean."
...
<center>
Observed difference = `r round(mean(HL$Horn_Length[HL$Group == "Alive"]), 2) - round(mean(HL$Horn_Length[HL$Group == "Dead"]), 2)` mm
</center>
...
```{r}
#| fig-align: center
#| fig-alt: "Histogram showing the difference in means for 100,000 simulations of two groups that are drawn from the same normal distribution. The +/- value for the observed difference of 2.2 is shown on either side with a vertical red line. The observations that equal or exceed this value are shown in red as points."
...
#| fig-align: center
#| fig-height: 7
#| fig-alt: "Density plot showing the relative likelihood versus values for the t distribution with 178 degrees of freedom. The tails of the distribution representing 2.5% of the density are shaded in pink."
...
#| fig-align: center
#| fig-height: 7
#| fig-alt: "Density plot showing the relative likelihood versus values for the t distribution with 178 degrees of freedom. The upper tail of the distribution representing 5% of the density is shaded in pink."
...
```{r echo=FALSE}
#| fig-alt: "Two histograms side by side. Each shows the distribution of 100,000 values drawn from a normal distribution with a mean of 5. The histogram on the left has a standard deviation of 1 while the histogram on the right has a standard deviation of 5."
...
#| echo: true
#| fig-align: center
#| fig-alt: "Density plot showing the relative likelihood versus values for a normal distributions. The tails of the distribution representing 2.5% of the density are shaded in pink."
...
#| echo: true
#| fig-align: center
#| fig-alt: "Density plot showing the relative likelihood versus values for the t distribution with 5 degrees of freedom. The tails of the distribution representing 2.5% of the density are shaded in pink."
...
#| fig-height: 5
#| fig-align: center
#| fig-alt: "Density plot showing the relative likelihood versus values for the t distribution. Different colored lines show the distribution for different degrees of freedom (2, 5, and 20). A dark blue line shows the density for a normal distribution."
...
> A confidence interval is a range that we expect, with some level of confidence, to include the true value of a population parameter such as the mean. [@Curran-Everett2009-zz]
...
#| echo: true
#| fig-align: center
#| fig-alt: "Density plot showing the relative likelihood versus values for the F distribution with 2 and 10 degrees of freedom. The tail of the distribution representing 5% of the density are shaded in pink."
...
#| echo: true
#| fig-align: center
#| fig-alt: "Density plot showing the relative likelihood versus values for the chi-squared distribution with 2 degrees of freedom. The tail of the distribution representing 5% of the density are shaded in pink."
...
```{r}
#| fig-height: 7
#| fig-alt: "Density plot showing the relative likelihood versus values for the normal distribution. The tails of the distribution representing 2.5% of the density are shaded in pink."
...
```{r}
#| fig-height: 7
#| fig-alt: "Density plot showing the relative likelihood versus values for the normal distribution. The upper tail of the distribution representing 5% of the density is shaded in pink."
...
#| echo: true
#| fig-align: center
#| fig-alt: "Density plot showing the relative likelihood versus values for a normal distribution with a mean of -10 and standard deviation of 2. The upper tail of the distribution representing 34% of the density is shaded in pink."
...
```{r}
#| fig-alt: "Density plot showing the relative likelihood versus values for the normal distribution."
...
```{r}
#| fig-alt: "Dotplot showing 50 random draws from the normal distribution. Points are stacked vertically when there is more than one value at the same point"
...
```{r}
#| fig-alt: "Density plot showing the relative likelihood versus values for the normal distribution. There is a vertical line at the x value of 1 and a dashed horizontal line at the y value 0.242, which is the relative likelihood corresponding to x=1."

M <- tibble(x = seq(-4, 4, length = 200), y = dnorm(x))
...
```{r}
#| echo: false
#| fig-alt: "Density plot showing the relative likelihood versus values for the normal distribution. The lower tail for x=1 is shaded in pink."
...
```{r}
#| fig-alt: "Scatterplot of Horn Length versus group (alive or dead). The effect size lower bound is plotted as a dark blue box, the estimate as a lighter blue box, and the upper bound as the lightest blue box."
...
```{r}
#| fig-alt: "Figure showing the estimated effect size and 95% confidence interval along the x axis for different experiments along the y axis. Darker colored points and lines denote higher p-values. Different regions of the plot show where an effect is not biologically meaningful, uncertain, or clearly biologically meaningful."
...
dec.plot <- ggplot(gall, aes(x, y, group = group, color = -log10(P))) +
  ...
  annotate(
    ...
    x = 0,
    y = 10.5,
    label = "Not Biologically\nMeaningful",
    size = 6
  ) +
  annotate("text", x = 0.35, y = 10.5, label = "Uncertain", size = 6) +
  annotate(
    "text",
    x = 0.75,
    y = 10.5,
    label = "Clearly Biologically\nMeaningful",
    size = 6
  ) +
  ylim(c(1, 11)) +
  theme(
...
```{r}
#| fig-alt: "Figure showing the estimated effect size and 95% confidence interval along the x axis for different experiments along the y axis. Red points and lines show significant results while black points and lines are not significant."
