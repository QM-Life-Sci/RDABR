...
LW <- read_excel("../data/Earth_Land_Water.xlsx") |> 
  ...
  relocate(Sample) |> 
  rowwise() |> 
  mutate(CI_lower = BinomCI(Water, n_Trials)[1, 2],
...
ggplot(data = tibble(Successes = 0:18,
...
ggplot(data = LW) +
  geom_bar(aes(x = Sample, y = `Percent Water`), stat = "identity",
           fill = "firebrick4", alpha = 0.5) +
  geom_errorbar(aes(x = Sample, y = `Percent Water`,
...
ggplot(data = LW) +
  geom_bar(aes(x = Sample, y = `Cumulative Percent Water`), stat = "identity",
           fill = "firebrick4", alpha = 0.5) +
  geom_errorbar(aes(x = Sample, y = `Percent Water`,
                    ymin = C_CI_lower, ymax = C_CI_upper),
                width = 0.25) +
  scale_x_continuous(breaks = 1:8)
