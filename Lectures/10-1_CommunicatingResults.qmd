---
title: "Unit 8: Summarizing models"
...
- PS 8 due after Spring Break (4/2)
- PC 2 after Spring break
    - Submit questions for discussion/review by midnight Monday 3/30 and midnight 4/1
    - PC 2 available Tuesday 3/31
...
![](./images/Picasso_3.png){fig-align="center" fig-alt="Image of Picasso work Guernica."}


## The Weeping Woman (1937)

![](./images/Picasso_4.png){fig-align="center" fig-alt="Image of Picasso work The Weeping Woman"}


## Self-portrait Facing Death (1972)

![](./images/Picasso_5.png){fig-align="center" fig-alt="Image of Picasso work Self-portrait Facing Death"}


## Untitled (1894)

![](./images/Picasso_1.png){fig-align="center" fig-alt="Image of Picasso work Untitled"}


## Untitled (1894)

![](./images/Picasso_2.png){fig-align="center" fig-alt="Image of Picasso work Untitled"}
...
![](./images/Picasso_6.png){fig-align="center" fig-alt="Image of a Picasso sketch from his notebook."}
...
#| echo: false
#| fig-align: center
#| fig-alt: "Histogram of the difference in means from simulated data. The distribution is centered on zero and ranges from approximately -2 to 2"
...
knitr::kable(
  as_tibble(sds, .name_repair = "unique") |>
...
t_tester <- function(m1, m2, sd1, sd2, ss = 10, niter = 100000) {
  ...
  for (jj in 1:niter) {
    fm <- t.test(rnorm(ss, m1, sd1), rnorm(ss, m2, sd2), var.equal = TRUE)
    ps[jj] <- fm$p.value < 0.05
  }
  return(mean(ps))
}

D <- tibble(m1 = 0, m2 = 0, sd1 = c(1, 1, 1, 1), sd2 = c(1, 2, 4, 16))
```
...
t_tester_welch <- function(m1, m2, sd1, sd2, ss = 10, niter = 100000) {
  ps <- logical(length = niter)
  for (jj in 1:niter) {
    mod <- t.test(rnorm(ss, m1, sd1), rnorm(ss, m2, sd2), var.equal = FALSE)
...
HL <- read_csv(
  "https://raw.githubusercontent.com/QM-Life-Sci/QMLS_1/refs/heads/main/data/HornedLizards.csv",
  show_col_types = FALSE
) |>
  drop_na() |>
  rename(`Horn Length` = horn_length, Group = group) |>
...
modelsummary(
  ...
  statistic = c("s.e. = {std.error}", "t = {statistic}", "p = {p.value}"),
  gof_omit = ".*"
)
```
...
```{r}
#| fig-alt: "Plot of the model term versus the coefficient estimate and 95% confidence interval."

modelplot(fm)
```


# Combining Datasets {background-color="#40666e"}

## Thinking about storing data

- Reduce redundancy
    - One big file vs. several smaller files
- Combine data programmatically


## Joining data

![](./images/Malurus_cyaneus.jpg){fig-align="center" fig-alt="Photo of a male and female supurb fairywren."}
...
- `full_join()`
    - Merge all rows, keep all values, add NA as needed
- Others but these are the only two you need
:::


## Thinking about what you want to keep

- Keep only rows in `df2` that match `df1`, duplicating information
- Keep all the rows in both `df1` and `df2`, adding `NA`s for missing data
