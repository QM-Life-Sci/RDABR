...
subtitle: "Data analysis workflow and R basics"
html-math-method:
  method: mathjax
...
Tues
- How is science verified?
    - show with sci paper activity (read as hw)
    - in class writing reflection (how is sci verified? What questions do i want to contribute to & how to do so rigorously)
- Introduce databases & concepts of open data & reproducible code
- Coding activity fetching from a database

Thurs
- PS together in groups (choose mascot)
- run code from paper?

- take away assignment - revise reflection (above ?s?) & coding check for understanding
...
![](images/BCS_1.png){fig-alt="Image showing the announcement of room temperature superconductivity."}


## September 26, 2022

![](images/BCS_2.png){fig-alt="Image showing the retraction of the paper about room temperature superconductivity."}


## 12 years to retraction

![](images/MMR_Lancet.png){fig-alt="Image showing the retraction of the article implicating vaccines in causing autism."}


## CRISPR-Cas9 (2012)

![](images/CRISPR.png){fig-alt="Image showing the announcement of CRISPR."}


## Nobel Prize (2020)

![](images/CRISPR_Nobel.png){fig-alt="Image showing the announcement of the 2020 nobel prize for CRISPR."}
...
![](images/Replication_crisis.png){fig-alt="Image showing the replication crisis in science."}
...
![](images/Cancer_Replicability_1.png){fig-alt="Image showing that most cancer studies can't be reproduced."}
...
![](images/Cancer_Replicability_2.png){fig-align="center" width="50%" fig-alt="Reproducibility project for cancer biology"}

![](images/Cancer_Replicability_3.png){fig-align="center" width="80%" fig-alt="Results of the Reproducibility project for cancer biology"}
...
2. **A**ccessibility
3. **I**nteroperability
4. **R**mutability (Reproducibility)
...
![](images/Prigent.png){fig-align="center" fig-alt="Image of FAIR principles for image data."}
...
![](images/Workflow.drawio.png){fig-align="center" width="80%" fig-alt="Flowchart showing ways to use the tidyverse for data analysis."}
...
(1|Year),data=tvdat); summary(tvtest)
ggplot(tvdat,aes(x=log(DD5_TOT),y=sqrt(tv),col=Region))+geom_point(shape=1,size=3,alpha=1/3)+geom_smooth(aes(col=Region),method='lm')+
...
![](images/earth.jpg){fig-align="center" fig-alt="Image of the Earth from space."}
...
Simulation is how we know that statistical tests "work" and are not biased (conservative or anti-conservative).
...
```{r}
#| fig-alt: "Scatter plot of Height vs. Age for 6,465 males."
...
```{r}
#| fig-alt: "Histogram of heights for those over age 20."
...
```{r}
#| fig-alt: "Density plot of a normal distribution overlaid over the histogram of heights."
...
ggplot(NHANES_20, aes(Height)) +
  ...
  geom_line(
    data = tibble(
      Height = seq(
        min(NHANES_20$Height),
        max(NHANES_20$Height),
        length.out = 100
      ),
      y = dnorm(
        Height,
        mean = mean(NHANES_20$Height),
...
MM <- data.frame(x = 1:3, y = c(10, 17, 21), A = c("a", "b", "c"))
MM
```
