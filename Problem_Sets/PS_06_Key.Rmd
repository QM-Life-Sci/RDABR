...
We want to first do a simulation to better understand what the *P*-value is a summary represents. Keep in mind that, for science that we do typically, we do not have the ability to do this kind of simulation where we know the answer ahead of time. 
...
stalk |> 
  ...
  summarize(mean_ES = mean(eye_span),
            SEM_ES = SEM(eye_span),
            Approx_Lower = mean_ES - 2 * SEM_ES,
            Approx_Upper = mean_ES + 2 * SEM_ES,
            df = length(eye_span) - 1,
            t_crit = abs(qt(p = 0.025, df = df)),
            Exact_Lower = mean_ES - t_crit * SEM_ES,
            Exact_Upper = mean_ES + t_crit * SEM_ES)
```
...
> The intercept is 1.95 - 2.14. The difference is -0.63 - -0.37. The CI for corn is narrower. This is because a different degrees of freedom is used here (43 vs. 20). The CI for the difference does not overlap with zero (and is very far from zero), which agrees with the *P*-value of 7 x 10^-10.
