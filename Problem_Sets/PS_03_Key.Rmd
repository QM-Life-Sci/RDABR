...
You should delete the first line and replace `x` in the plotting statment to be your vector created above.
...
In lecture we looked at the interquartile range (distance between the 0.25 and 0.75 quantiles) as a way to describe the spread of data. In this exercise we will explore how to use quantiles to summarize distributions.

Start be generating 10000 random normal values (call it `x` for convenience). It is best to use a mean of 0 and sd of 1 (the defaults for `rnorm()`).
...
These vectors are TRUE and FALSE values. Print out the first 10 of each to confirm.
...
Add the two sums together and divide by 10000. Confirm that this value matches the expected percent of observations more than 1 standard deviation from the mean.
...
How close do they match? If they are off, why might that be?
...
You can use either. Just comment the other out.
...
At the end of lecture 3-2, we used the Law of Large Numbers to show how the mean of a set of samples from a population is a good estimate of the population's mean. That code, condensed into one chuck is:
