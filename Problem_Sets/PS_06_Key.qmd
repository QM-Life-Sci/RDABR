...
We have set the seed for you, so that our answers will match.
...
# This satisfies the "observed test statistic" part of the hypothesis
# definition.
d[1] <- d_obs

# Run the randomization
for (ii in 2:iters) {
  # Iterate from 2 to iters
...
#| fig-alt: "A histogram of the difference between groups for 10,000 simulations."

ggplot(data = tibble(d), aes(d)) +
...
#| fig-alt: "A histogram of the difference between groups for 10,000 simulations with vertical lines showing where +/- the observed difference lies."

ggplot(tibble(d), aes(d)) +
  geom_histogram(bins = 30) +
  geom_vline(
    xintercept = c(d_obs, -d_obs),
    color = "hotpink",
    linewidth = 2
  )
...
DD <- tibble(x, Grp = rep(c("x1", "x2"), each = 20))
DD
```
...
Compare the hypothesis test for the difference in means between `x1` and `x2` for the linear model (the `Grp2` coefficient, SE, *t*-value, and *P*-value). Are they similar or different? If they are similar, but not exactly the same, what do you think might explain this discrepancy?
...
```{r}
#| fig-alt: "A scatterplot of lion age versus the proportion black on the nose."
...
What is your interpretation of the *slope* of the regression in mathematical terms (i.e., "For a 1 unit increase in _____, _____ increases by _____.")?
