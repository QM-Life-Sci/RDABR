...
```{r}
#| fig-alt: "Histogram with a mean of 5 showing a broadly normal distribution with a range of approximately 4 to 6."
...
```{r}
#| fig-alt: "Histogram with a mean of 5 showing a broadly normal distribution with a range of approximately 4 to 6. The bins are very narrow."
...
> The shape changes somewhat. When there are only 5 bins, each one represents a large range of the data. It looks vaguely normal, but it's hard to tell what the distribution really looks like. When the number of bins is very high, the bins are really narrow, and the plot looks very sporadic.
...
```{r}
#| fig-alt: "A line plot with a single density line with a normal (bell) shape."
...
SF <- read_csv(
    ...
    mutate(family = factor(family), block = factor(block)) |>
...
```{r}
#| fig-alt: "A 4 x 4 panel of histograms for each population as a different panel. Most all are normally distributed but some have extreme values."
...
```{r}
#| fig-alt: "Histogram of 1000 numbers with a mean of 10 and sd of 4 showing a largely normal distribution. The range is approximately 8 to 11."
...
```{r}
#| fig-alt: "Histogram of 1000 numbers with a mean of 10 and sd of 4 showing a largely normal distribution. The range is approximately 9 to 10.5."
...
```{r}
#| fig-alt: "Histogram of 1000 numbers with a mean of 10 and sd of 4 showing a largely normal distribution. The range is approximately 7 to 12."
