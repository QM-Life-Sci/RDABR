...
Cons <- read_csv(
  "../data/Spp_conservation_status.csv",
...
Below we copied several code chunks from the `Manuscript_Template.qmd` file. Replace then demo code with your code to make a figure or figures and one or more tables and summaries.
...
#| fig_alt: "Scatterplot of lion age vs. the proportion of black coloration in the lion's nose."
...
#| fig-alt: "Scatterplot showing eyespan measured in stalk-eyed flies raised on two different food treatments: corn and cotton."
...
datasummary_skim(
  LA |>
    rename(`Age (y)` = age, `Proportion Black` = proportion.black),
  fun_numeric = list(
    n = N,
...
#| message: false
#| tbl-cap: "Summary statistics for observations of lion age and proportion of black coloration in the nose."
datasummary_balance(~food_source, data = stalk)
```
...
modelsummary(
  list("Age ~ Proportion Black" = fm_LA),
  estimate = "{estimate} ({conf.low}, {conf.high})",
  statistic = c("t = {statistic}", "df = {df.error}", "p = {p.value}"),
  gof_omit = ".*",
  fmt = 2
)
```
...
modelsummary(
  list("Eyespan ~ Food Source" = fm_stalk),
  estimate = "{estimate} ({conf.low}, {conf.high})",
  statistic = c("t = {statistic}", "df = {df.error}", "p = {p.value}"),
  gof_omit = ".*",
  fmt = 2
...
#| fig-alt: "Coefficient plot for the linear model predicting lion age by proportion of black nose coloration."
modelplot(fm_LA)
```
...
#| fig-alt: "Coefficient plot for the linear model predicting eyespan in stalk-eyed flies by food source."
modelplot(fm_stalk)
```
