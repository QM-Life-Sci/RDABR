...
Follow the example in the lecture slides to carry out *leave-one-out* cross validation on these models using the `cvFit()` function from the `cvTools()` package. Cross-validation uses the data to determine the root-mean-squared prediction error for a model (RMSE). By default, `cvFit()` use RMSE as its "cost" function.
