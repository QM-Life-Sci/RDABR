...
The `test` parameter is an equality or inequality (==, <, >, etc.) which returns a `TRUE` or `FALSE`. For each element, if the result is TRUE, the value at `yes` is returned. If the result is FALSE, the value at `no` is returned. Study the following examples.
