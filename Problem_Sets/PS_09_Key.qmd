...
ggplot(HT, aes(x = Mismatch_Degree, y = Survival_Days)) +
  geom_point(
    position = position_jitter(width = 0.05, seed = 384729),
    size = 3,
    alpha = 0.5
  ) +
...
> The good and medium quality match groups are strongly right skewed. One of the assumptions of ANOVA is equal variance across the groups. We could explicitly test this assumption, but there isn't any reason to. The test will be significant for inequality. So we should just transform and go from there.
...
# FIXME
fm <- lm(
  N_Species ~ Dist_to_Island + Elevation + Area + Dist_to_Ecuador,
  data = PB
)
```
