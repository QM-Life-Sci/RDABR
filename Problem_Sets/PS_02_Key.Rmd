...
## RMarkdown $\rightarrow$ HTML

We will be working in [RMarkdown files](https://rmarkdown.rstudio.com/index.html). Markdown is a markup language that is designed to be no only readable in its text form, but also able to be converted into other formats. Markdown has a simple syntax which allows for things like bold ("**Activities**" above), *italics*, and facilities for including math notation ($\alpha, \beta, \gamma, x^2$). Even some pretty fancy math using [LaTeX](https://www.latex-tutorial.com/):

$$\bar{Y} = \frac{\sum^{n}_{i = 1}Y_i}{n}$$

RMarkdown is the marriage of R and [markdown](https://en.wikipedia.org/wiki/Markdown), allowing you to write text and R code together that can include analysis, figures, and tables. Markdown has been extended for making slides (like the ones we use in this class), adding references, bibliographies, and cross references.

Our usage in this class will be fairly pedestrian, but you can do some really complex things, like writing entire manuscripts, making a CV, making slides (all the slides for this course are knitted from markdown files), or generate a website.

RMarkdown files can be converted to different formats. We will use HTML (PDF and Word are other options). 

- Click "Knit" at the top of this window right now.

You might get a message about installing some packages. Click yes to install the required packages. After a few seconds, another window should open with this document knitted as a visually pleasing file.

You have just compiled an RMarkdown file into HTML. These Rmd and HTML files will be the basic currency of the problems sets and progress checks you will do in this class.
...
You can also run code interactively from a Markdown file to the R console. To run a single line, press *Ctrl-Enter* / *Cmd-Return*. To run the current chunk, use *Ctrl-Alt-c* / *Cmd-Shift-Return*. This is a good way to test code that you are working on, rather then waiting to compile the file to HTML (or whatever format you are using).
...
If you want to take the mean of a vector `x`, we recommend using `mean_x`, `x_bar`, or `x_mean`.^[I prefer the latter two because of RStudio's auto-completion feature.] There are two benefits of using one of these variable names over using `mean`.
...
In contrast, numbers^[If you have experience with this, we are talking about floating point numbers.] are often rounded to some level of accuracy (R uses about 53 decimal places). For example:
...
# FIXME
set.seed(37)
M6_by_6 <- matrix(rnorm(36, mean = 10, sd = 5), nrow = 6)
M6_by_6
```
