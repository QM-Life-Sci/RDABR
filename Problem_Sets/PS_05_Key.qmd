...
2. Plot the raw data (points) for the two groups. There are many example of this kind of plot in the lecture slides.
...
#| fig-alt: "A boxplot of Eye span versus Food Source. Eye span is longer on average and less variable on the corn food source compared to the cotton food source."
...
```{r}
#FIXME
#| fig-alt: "A scatterplot of Eye span versus Food Source with means and standard errors overlayed in purple. Eye span is longer on average and less variable on the corn food source compared to the cotton food source."
...
We will investigate the other parts of this summary in the coming units.

Next, we will use a *t*-test to estimate the difference in means in the two treatments. A *t*-test is a very specific kind of linear model which compares the means of two (and only two) groups. One of the assumptions of a *t*-test is that the variances of the two groups are equal.
...
> Corn is the base level. The table has a line for the Intercept, which represents the mean for Corn. The other line is the difference for cotton.
...
The `test` parameter is an equality or inequality (==, <, >, etc.) which returns a `TRUE` or `FALSE`. For each element, if the result is TRUE, the value at `yes` is returned. If the result is FALSE, the value at `no` is returned. Study the following examples, and try out some other `ifelse`s.
...
stalk <- stalk |>
  mutate(
    food01 = ifelse(food_source == "Corn", 0, 1),
